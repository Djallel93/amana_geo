...
function getAllSecteurs() {
    ...
    const secteurs = data.slice(1).map(row => ({
        id: row[CONFIG.COLUMNS.SECTEURS.ID],
        nom: row[CONFIG.COLUMNS.SECTEURS.NOM],
        ...
        idVille: row[CONFIG.COLUMNS.SECTEURS.ID_VILLE]
    })).filter(s => s.id);
...
function updateSecteur(id, updates) {
    ...
    const rowIndex = data.findIndex(row => row[CONFIG.COLUMNS.SECTEURS.ID] == id);
    ...
    if (updates.nom) {
        sheet.getRange(actualRow, CONFIG.COLUMNS.SECTEURS.NOM + 1).setValue(updates.nom);
    }
    if (updates.latitude !== undefined) {
        sheet.getRange(actualRow, CONFIG.COLUMNS.SECTEURS.LATITUDE + 1).setValue(updates.latitude);
    }
    if (updates.longitude !== undefined) {
        sheet.getRange(actualRow, CONFIG.COLUMNS.SECTEURS.LONGITUDE + 1).setValue(updates.longitude);
    }
    if (updates.idVille) {
        sheet.getRange(actualRow, CONFIG.COLUMNS.SECTEURS.ID_VILLE + 1).setValue(updates.idVille);
    }
